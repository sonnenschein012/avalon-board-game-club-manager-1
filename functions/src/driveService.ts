...
export interface SheetInspectionResult {
  ...
  title: string;
  tabs: Array<{ sheetId: number; title: string; index: number }>;
  defaultTabId: number;
  errorMessage?: string;
}

export interface SheetAttendanceDataResult {
  success: boolean;
  spreadsheetId: string;
  sheetId: number;
  tabTitle: string;
 ...
 */
export async function validateFormTemplateFile(
  fileId: string,
  authClient: OAuth2ClientInstance
): Promise<FormTemplateValidationResult> {
 ...
 */
export async function fetchSpreadsheetValuesByTabId(
  spreadsheetId: string,
  sheetId: number,
  authClient: OAuth2ClientInstance
): Promise<SheetAttendanceDataResult> {
  const sheets = google.sheets({ version: 'v4', auth: authClient });
  ...
  try {
    ...
    const matchedSheet = (metaRes.data.sheets || []).find(
    ...
    const rawValues = (valuesRes.data.values || []) as string[][];
