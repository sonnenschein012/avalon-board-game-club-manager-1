...
export default function MeetingRecordsModal({ onClose, onView, members }: { onClose: () => void; onView: (date: string) => void; members: Member[] }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/60 p-3 backdrop-blur-sm" onKeyDown={event => {
    ...
    <section role="dialog" aria-modal="true" aria-label="모임 진행 기록 관리" className="flex max-h-[85dvh] w-full max-w-2xl flex-col rounded-3xl bg-white p-5 shadow-xl">
      <div className="flex items-start justify-between gap-4">
        <div><h2 className="text-lg font-black text-navy">모임 진행 기록 관리</h2><p className="mt-2 text-sm text-slate-500">날짜별 조 구성·음료·희망사항을 열람하거나 삭제합니다. 세션 기록은 유지됩니다.</p></div>
