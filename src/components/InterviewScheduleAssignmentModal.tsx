...
export default function InterviewScheduleAssignmentModal({ open, applicantsCount, alreadyScheduledCount = 0, schedules, saving = false, onClose, onAssign, onCreateSchedule }: Props) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 backdrop-blur-sm">
    <div role="dialog" aria-modal="true" aria-label="면접 일정 지정" className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-lg flex-col overflow-hidden rounded-3xl bg-white p-5 shadow-2xl">
      <div className="flex shrink-0 items-start justify-between gap-3">
        <div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gold">{applicantsCount}명 선택됨</p><h2 className="mt-1 text-lg font-black text-navy">면접 일정 지정</h2><p className="mt-1 text-sm leading-5 text-slate-500">지원자를 넣을 기존 일정을 선택하거나 새 일정을 만드세요.</p></div>
      ...
      {alreadyScheduledCount > 0 && <p className="mt-4 shrink-0 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs font-bold leading-5 text-amber-800">선택한 지원자 중 {alreadyScheduledCount}명은 이미 면접 일정이 지정되어 있습니다. 다른 일정으로 옮기면 응답과 시간 배정이 초기화됩니다.</p>}
