...
interface ArchiveWidgetCorePlayersProps {
  ...
  corePlayerIncludeBoardMembers: boolean;
  setCorePlayerIncludeBoardMembers: (b: boolean) => void;
  corePlayerGenres: string[];
  setCorePlayerGenres: (g: string[]) => void;
  corePlayerDifficulties: string[];
  setCorePlayerDifficulties: (d: string[]) => void;
  corePlayerGameIds: string[];
  setCorePlayerGameIds: (g: string[]) => void;
  corePlayerGameSearch: string;
  setCorePlayerGameSearch: (q: string) => void;
  games: Game[];
  corePlayers: {id: string, hits: number, member?: Member | undefined}[];
...
export default function ArchiveWidgetCorePlayers({
  selectedSemester,
  corePlayerIncludeBoardMembers,
  setCorePlayerIncludeBoardMembers,
  corePlayerGenres,
  setCorePlayerGenres,
  corePlayerDifficulties,
  setCorePlayerDifficulties,
  corePlayerGameIds,
  setCorePlayerGameIds,
  corePlayerGameSearch,
  setCorePlayerGameSearch,
  games,
  corePlayers,
  availableGenres,
  difficultyRanges
    ...
    <div className="glass-panel p-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-4">
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            <input 
              type="checkbox" 
              checked={corePlayerIncludeBoardMembers}
              onChange={e => setCorePlayerIncludeBoardMembers(e.target.checked)}
      ...
      <div className="space-y-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
        <div>
           ...
           <FilterPills options={availableGenres} selected={corePlayerGenres} onChange={setCorePlayerGenres} />
        </div>
        <div>
           <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">대상 난이도 필터</p>
           <FilterPills options={difficultyRanges.map(r => r.label)} selected={corePlayerDifficulties} onChange={setCorePlayerDifficulties} />
        </div>
        <div>
          ...
          <div className="space-y-2 relative">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="게임 검색..."
                value={corePlayerGameSearch}
                onChange={e => setCorePlayerGameSearch(e.target.value)}
            ...
            {corePlayerGameSearch && (
              <div className="absolute top-full left-0 right-0 mt-1 p-2 bg-white rounded-lg border border-slate-200 shadow-xl max-h-48 overflow-y-auto z-10">
                {games
                   .filter(g => g.title.toLowerCase().includes(corePlayerGameSearch.toLowerCase()))
                   .map(g => (
                     <button
                       key={g.id}
                       onClick={() => {
                         if (!corePlayerGameIds.includes(g.id)) {
                           setCorePlayerGameIds([...corePlayerGameIds, g.id]);
                         }
                         setCorePlayerGameSearch('');
                       }}
                ...
                {games.filter(g => g.title.toLowerCase().includes(corePlayerGameSearch.toLowerCase())).length === 0 && (
                  <p className="text-xs text-slate-400 text-center py-4">검색 결과가 없습니다.</p>
                )}
              </div>
            )}
            
            {corePlayerGameIds.length > 0 && (
              <div className="flex flex-wrap gap-2 pt-2">
                {corePlayerGameIds.map(id => {
                  ...
                  return (
                    <div key={id} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg border border-blue-100 shadow-sm text-sm font-bold">
                      {game.title}
                      <button 
                        onClick={() => setCorePlayerGameIds(corePlayerGameIds.filter(gId => gId !== id))}
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
        ...
        {corePlayers.length === 0 && <p className="text-center text-slate-400 text-sm py-8 font-bold">표시할 데이터가 없습니다.</p>}
      </div>
    </div>
  );
}
