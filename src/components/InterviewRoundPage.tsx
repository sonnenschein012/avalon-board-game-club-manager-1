...
export default function InterviewRoundPage({ isAdminModeActive = false }: { isAdminModeActive?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {tab === 'settings' && <section className="rounded-3xl bg-white p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><h3 className="font-black text-navy">회차 공통 설정</h3><p className="mt-1 text-sm text-slate-500">면접 질문과 메시지 템플릿, 새 일정에 사용할 기본값을 관리합니다.</p><p className="mt-2 text-xs font-bold text-indigo-600">실제 조사 기간과 면접 가능일은 각 면접 일정에서 설정합니다.</p></div><button onClick={() => setSettingsOpen(true)} className="flex items-center gap-2 rounded-xl bg-navy px-4 py-2.5 text-xs font-black text-white"><Settings size={15} />설정 변경</button></div></section>}
