...
export default function InterviewRoundPage({ isAdminModeActive = false }: { isAdminModeActive?: boolean }) {
  ...
  return <div className="space-y-6">
    ...
    {tab === 'interviewers' && <section className="space-y-4"><div className="flex flex-col gap-3 rounded-3xl bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-black text-navy">면접관 관리</h2><p className="mt-1 text-xs text-slate-400">명부의 기본 정보와 일정별 가능시간을 나누어 관리합니다.</p></div><InterviewScheduleSelector schedules={logic.schedules.filter(schedule => schedule.status !== 'archived')} activeScheduleId={activeSchedule?.id ?? null} applicantCounts={scheduleApplicantCounts} allowNone noneLabel="면접관 명부" onSelect={logic.setActiveScheduleId} /></div><InterviewersPanel round={scheduleViewRound} interviewers={logic.activeInterviewers} roster={logic.interviewers} schedules={logic.schedules} applicants={activeSchedule ? scheduleApplicants : logic.applicants} isRoster={!activeSchedule} onAdd={logic.addInterviewer} onEdit={logic.editInterviewer} onSaveAvailability={logic.saveInterviewerAvailability} onRemove={logic.removeInterviewer} onAssign={logic.assignInterviewer} /></section>}
