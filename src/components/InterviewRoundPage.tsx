...
import { Archive, ArrowDownAZ, ArrowLeft, ArrowUpAZ, CalendarClock, Copy, Download, FileUp, Loader2, Lock, MessageSquare, Search, Settings, Unlock, UserPlus, Users } from 'lucide-react';
...
type Tab = 'overview' | 'applicants' | 'availability' | 'interviewers' | 'assignment' | 'dashboard' | 'settings';
const TABS: Array<{ id: Tab; label: string }> = [
  { id: 'overview', label: '개요' }, { id: 'applicants', label: '지원자' }, { id: 'availability', label: '지원자 가능시간' }, { id: 'interviewers', label: '면접관' }, { id: 'assignment', label: '일정 배정' }, { id: 'dashboard', label: '면접관 대시보드' }, { id: 'settings', label: '설정' },
];
const FILTERS: Array<{ id: InterviewApplicantFilter; label: string }> = [
  { id: 'all', label: '전체' }, { id: 'responded', label: '응답완료' }, { id: 'pending', label: '미응답' }, { id: 'assigned', label: '배정완료' }, { id: 'unassigned', label: '미배정' }, { id: 'availability-unsent', label: '조사 미발송' }, { id: 'availability-sent', label: '조사 발송' }, { id: 'availability-sent-pending', label: '조사 발송 후 미응답' }, { id: 'confirmation-unsent', label: '배정 후 확정 미발송' }, { id: 'confirmation-sent', label: '확정 발송' }, { id: 'archived', label: '보관됨' },
];
...
export default function InterviewRoundPage() {
  ...
  const counts = useMemo(() => ({
    total: logic.applicants.filter(item => (item.lifecycle ?? 'active') === 'active').length,
    responded: logic.applicants.filter(item => (item.lifecycle ?? 'active') === 'active' && item.access?.submittedAt).length,
    assigned: logic.applicants.filter(item => (item.lifecycle ?? 'active') === 'active' && item.assignment).length,
  }), [logic.applicants]);
  ...
  return <div className="space-y-6">
    <div><Link to="/interviews" className="mb-2 inline-flex items-center gap-1 text-xs font-bold text-slate-400 hover:text-navy"><ArrowLeft size={14} />면접 회차 목록</Link><PageHeader title={round.name} subtitle="Operations / Interview Round" icon={CalendarClock} stats={{ label: '지원자', value: counts.total }} actions={<div className="flex flex-wrap gap-2"><button onClick={exportApplicants} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-bold text-navy"><Download size={15} />CSV</button><button onClick={() => { setEditingApplicantId(null); setApplicantFormOpen(true); }} className="flex items-center gap-2 rounded-xl border border-navy px-3 py-2.5 text-xs font-black text-navy"><UserPlus size={15} />개별 추가</button><button onClick={() => setImportOpen(true)} className="flex items-center gap-2 rounded-xl bg-navy px-3 py-2.5 text-xs font-black text-white hover:bg-gold"><FileUp size={15} />CSV 병합</button></div>} /></div>
    ...
    {tab === 'assignment' && <section className="space-y-4"><AutoAssignmentPanel round={round} applicants={logic.applicants} interviewers={logic.interviewers} draft={logic.autoDraft} onRun={mode => { logic.runAutoAssignment(mode); }} onDraftChange={logic.setAutoDraft} onApply={logic.applyAutoDraft} /><div className="rounded-2xl bg-indigo-50 px-4 py-3 text-xs leading-5 text-navy"><strong>수동 배정:</strong> 운영진이 직접 저장한 일정은 기본적으로 잠깁니다. 면접관과 지원자의 가능시간이 모두 겹치는 시간만 선택할 수 있습니다.</div>{logic.applicants.filter(applicant => (applicant.lifecycle ?? 'active') === 'active').map(applicant => {
      const selectedInterviewer = logic.interviewers.find(item => item.active && item.interviewerId === selectedInterviewerId);
      const applicantCandidates = new Set(availabilityToAssignmentCandidates(applicant.access?.availability ?? [], round.availabilitySlotMinutes, round.assignmentSlotMinutes));
      const candidates = selectedInterviewer ? availabilityToAssignmentCandidates(selectedInterviewer.availability, round.availabilitySlotMinutes, round.assignmentSlotMinutes).filter(slot => applicantCandidates.has(slot)) : [];
      return <article key={applicant.id} className="rounded-2xl bg-white p-4 shadow-sm"><div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><p className="font-black text-navy">{applicant.name}</p><p className="text-[11px] text-slate-400">{applicant.assignment ? `배정: ${formatAssignment(applicant)} · ${applicant.assignment.durationMinutes}분 · ${applicant.assignment.status}` : '미배정'}</p></div><div className="flex min-w-0 flex-wrap gap-2"><button onClick={() => logic.runAutoAssignment('applicant', applicant.id)} className="rounded-xl bg-indigo-50 px-3 py-2 text-xs font-bold text-navy">이 지원자 자동 추천</button><select value={assigningApplicantId === applicant.id ? selectedInterviewerId : ''} onChange={event => { setAssigningApplicantId(applicant.id); setSelectedInterviewerId(event.target.value); setSelectedSlot(null); }} className="rounded-xl border border-slate-200 px-3 py-2 text-xs"><option value="">면접관 선택</option>{logic.interviewers.filter(item => item.active).map(item => <option key={item.id} value={item.interviewerId}>{item.displayName}</option>)}</select><select value={assigningApplicantId === applicant.id ? selectedSlot ?? '' : ''} onChange={event => { setAssigningApplicantId(applicant.id); setSelectedSlot(event.target.value); }} className="min-w-[170px] flex-1 rounded-xl border border-slate-200 px-3 py-2 text-xs"><option value="">공통 가능시간</option>{candidates.map(slot => { const conflict = selectedInterviewer ? logic.getAssignmentConflict(slot, applicant.id, selectedInterviewer.interviewerId) : null; return <option key={slot} value={slot} disabled={Boolean(conflict)}>{formatSlot(slot)}{conflict ? ` · 배정됨 (${conflict.name})` : ''}</option>; })}</select><button disabled={assigningApplicantId !== applicant.id || !selectedSlot || !selectedInterviewerId} onClick={async () => { if (!selectedSlot) return; const saved = await logic.assignApplicant(applicant, selectedSlot, selectedInterviewerId, true); if (saved) { setSelectedSlot(null); setAssigningApplicantId(''); } }} className="rounded-xl bg-navy px-3 py-2 text-xs font-bold text-white disabled:opacity-40">배정·잠금</button>{applicant.assignment && <><button title={applicant.assignment.locked ? '잠금 해제' : '잠금'} onClick={() => void logic.changeAssignmentState(applicant.id, { locked: !applicant.assignment?.locked })} className="rounded-xl bg-amber-50 p-2 text-amber-700">{applicant.assignment.locked ? <Lock size={14} /> : <Unlock size={14} />}</button><button onClick={() => logic.clearAssignment(applicant.id)} className="rounded-xl bg-red-50 px-3 py-2 text-xs font-bold text-red-600">해제</button></>}</div></div></article>;
    })}</section>}

    {tab === 'dashboard' && <InterviewerDashboard round={round} interviewers={logic.interviewers} applicants={logic.applicants} changeRequests={logic.changeRequests} onStateChange={(applicantId, status) => logic.changeAssignmentState(applicantId, { status })} onResolveRequest={logic.resolveChangeRequest} onReassign={(applicant, slotId, interviewerId) => logic.assignApplicant(applicant, slotId, interviewerId, true)} />}
...
function ApplicantRow({ applicant, onOpen, onEdit, onArchive }: { applicant: InterviewApplicantWithAccess; onOpen: () => void; onEdit: () => void; onArchive: () => void }) {
  ...
  return <article className="flex w-full flex-col gap-3 rounded-2xl bg-white p-4 shadow-sm transition hover:bg-indigo-50 sm:flex-row sm:items-center">
    ...
    <div className="flex shrink-0 gap-2"><button onClick={onEdit} className="rounded-xl bg-slate-100 px-2.5 py-2 text-[10px] font-bold text-navy">수정</button><button onClick={onArchive} title={(applicant.lifecycle ?? 'active') === 'archived' ? '복원' : '보관'} className="rounded-xl bg-amber-50 p-2.5 text-amber-700"><Archive size={14} /></button><button onClick={() => navigator.clipboard.writeText(applicant.link).then(() => toast.success('링크를 복사했습니다.')).catch(() => toast.error('링크를 복사하지 못했습니다.'))} title="개인 링크 복사" className="rounded-xl bg-slate-100 p-2.5 text-slate-500"><Copy size={14} /></button><button onClick={onOpen} className="flex items-center gap-1 rounded-xl bg-navy px-3 py-2 text-[11px] font-bold text-white"><MessageSquare size={14} />문자</button></div>
  </article>;
}
