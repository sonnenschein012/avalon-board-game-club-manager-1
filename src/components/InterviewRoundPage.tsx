...
import { Archive, ArrowDownAZ, ArrowLeft, ArrowUpAZ, CalendarClock, Copy, Download, FileUp, KeyRound, Loader2, MessageSquare, RotateCcw, Search, Settings, UserMinus, UserPlus, Users } from 'lucide-react';
...
export default function InterviewRoundPage() {
  ...
  return <div className="space-y-6">
    ...
    {tab === 'selection' && <SelectionPanel round={round} applicants={logic.applicants} onUpdateOverallRating={logic.updateCompletedRating} onUpdateSelectionStatus={logic.updateSelectionStatus} />}
...
function ApplicantRow({ applicant, onOpen, onEdit, onArchive, onWithdraw, onReset, onReissue }: { applicant: InterviewApplicantWithAccess; onOpen: () => void; onEdit: () => void; onArchive: () => void; onWithdraw: () => void; onReset: () => void; onReissue: () => void }) {
  ...
  const confirmationIsCurrent = Boolean(applicant.assignment) && Boolean(applicant.confirmationMessage.lastMarkedSentAt)
  ...
  return <article className="flex w-full flex-col gap-3 rounded-2xl bg-white p-4 shadow-sm transition hover:bg-indigo-50 sm:flex-row sm:items-center">
    <button onClick={onOpen} className="flex min-w-0 flex-1 flex-col gap-3 text-left lg:flex-row lg:items-center lg:justify-between">
      <div className="flex min-w-[180px] items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-navy"><Users size={16} /></span><div><p className="font-black text-navy">{applicant.name} <span className="text-xs font-normal text-slate-400">{applicant.applicantNumber}</span>{withdrawn && <span className="ml-1 rounded-full bg-red-50 px-2 py-0.5 text-[9px] text-red-700">지원 철회</span>}</p><p className="text-[11px] text-slate-400">{applicant.phone}</p></div></div>
    ...
    <div className="flex shrink-0 flex-wrap gap-2"><button onClick={onEdit} className="rounded-xl bg-slate-100 px-2.5 py-2 text-[10px] font-bold text-navy">수정</button><button onClick={onWithdraw} title={withdrawn ? '지원 철회 취소' : '지원 철회'} className={`rounded-xl p-2.5 ${withdrawn ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}><UserMinus size={14} /></button><button disabled={withdrawn} onClick={onReset} title={withdrawn ? '철회 취소 후 일정 초기화 가능' : '일정 초기화'} className="rounded-xl bg-indigo-50 p-2.5 text-indigo-600 disabled:cursor-not-allowed disabled:opacity-35"><RotateCcw size={14} /></button><button disabled={withdrawn} onClick={onReissue} title="개인 링크 재발급" className="rounded-xl bg-violet-50 p-2.5 text-violet-700 disabled:opacity-35"><KeyRound size={14} /></button><button onClick={onArchive} title={(applicant.lifecycle ?? 'active') === 'archived' ? '복원' : '보관'} className="rounded-xl bg-amber-50 p-2.5 text-amber-700"><Archive size={14} /></button><button disabled={withdrawn} onClick={() => navigator.clipboard.writeText(applicant.link).then(() => toast.success('링크를 복사했습니다.')).catch(() => toast.error('링크를 복사하지 못했습니다.'))} title={withdrawn ? '지원 철회로 링크가 비활성화됨' : '개인 링크 복사'} className="rounded-xl bg-slate-100 p-2.5 text-slate-500 disabled:opacity-35"><Copy size={14} /></button><button onClick={onOpen} className="flex items-center gap-1 rounded-xl bg-navy px-3 py-2 text-[11px] font-bold text-white"><MessageSquare size={14} />문자</button></div>
  </article>;
}
