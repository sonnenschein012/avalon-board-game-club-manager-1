...
function statusLabel(schedule: InterviewSchedule) {
  ...
  if (schedule.status === 'draft') return { label: '준비 중', className: 'bg-indigo-50 text-indigo-700' };
...
export default function InterviewScheduleSelector({ schedules, activeScheduleId, applicantCounts = {}, allowNone = false, noneLabel = '일정 선택', onSelect }: Props) {
  ...
  return <div ref={rootRef} className="relative min-w-0 sm:min-w-72">
    <button type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen(current => !current)} className="flex w-full items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-left shadow-sm transition hover:border-indigo-200 hover:bg-indigo-50/40">
      <span className="rounded-xl bg-indigo-50 p-2 text-indigo-700"><CalendarDays size={16} /></span>
    ...
    {open && <div role="listbox" className="absolute right-0 z-40 mt-2 w-[min(22rem,calc(100vw-2rem))] space-y-1 rounded-2xl border border-slate-100 bg-white p-2 shadow-2xl">
      {allowNone && <button type="button" role="option" aria-selected={!active} onClick={() => { onSelect(null); setOpen(false); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left ${!active ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}><span className="min-w-0 flex-1"><strong className="block text-xs font-black text-navy">{noneLabel}</strong><small className="text-[10px] text-slate-400">회차에 공통으로 사용할 면접관</small></span>{!active && <Check size={15} className="text-indigo-700" />}</button>}
      {schedules.map(schedule => { const status = statusLabel(schedule); const selected = schedule.id === activeScheduleId; return <button type="button" role="option" aria-selected={selected} key={schedule.id} onClick={() => { onSelect(schedule.id); setOpen(false); }} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition ${selected ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><strong className="truncate text-xs font-black text-navy">{schedule.name}</strong><span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black ${status.className}`}>{status.label}</span></span><small className="mt-1 block text-[10px] font-bold text-slate-400">{dateRange(schedule)} · 지원자 {applicantCounts[schedule.id] ?? 0}명</small></span>{selected && <Check size={15} className="shrink-0 text-indigo-700" />}</button>; })}
