...
interface Props {
  ...
  /** Starts the whole pending-applicant auto-assignment draft. */
  onRunAutoAssignment?: () => void;
  onRunApplicantAutoAssignment: (applicantId: string) => void;
  onApplyDraft: () => Promise<boolean>;
  onAssign: (applicant: InterviewApplicantWithAccess, slot: string, interviewerId: string) => Promise<boolean>;
...
function WeeklySchedule({ round, applicants, assigned, interviewers, draft, actionNeededIds, onDraftChange, onAssign, onClearAssignment, onChangeAssignmentState, onResetSchedule }: { round: InterviewRound; applicants: InterviewApplicantWithAccess[]; assigned: InterviewApplicantWithAccess[]; interviewers: InterviewRoundInterviewer[]; draft: AutoAssignmentResult | null; actionNeededIds: Set<string>; onDraftChange: Props['onDraftChange']; onAssign: Props['onAssign']; onClearAssignment: Props['onClearAssignment']; onChangeAssignmentState: Props['onChangeAssignmentState']; onResetSchedule: Props['onResetSchedule'] }) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    void onAssign(entry.applicant, targetSlot, assignment.interviewerId).then(success => {
...
export default function InterviewSchedulePanel({ round, applicants, interviewers, changeRequests, draft, onDraftChange, onRunAutoAssignment = () => undefined, onRunApplicantAutoAssignment, onApplyDraft, onAssign, onClearAssignment, onChangeAssignmentState, onResetSchedule }: Props) {
  ...
  const [visibleWaitingCount, setVisibleWaitingCount] = useState(SCHEDULE_LIST_BATCH);
  const activeInterviewers = useMemo(() => interviewers.filter(item => item.active), [interviewers]);
  const openChangeRequestApplicantIds = useMemo(() => new Set(changeRequests.filter(item => item.status === 'open').map(item => item.applicantId)), [changeRequests]);
  const waiting = useMemo(() => applicants.filter(item => activeApplicant(item) && item.access?.submittedAt && !item.assignment), [applicants]);
  const visibleWaiting = useMemo(() => waiting.slice(0, visibleWaitingCount), [visibleWaitingCount, waiting]);
  const assigned = useMemo(() => applicants.filter(item => activeApplicant(item) && item.assignment).sort((left, right) => (left.assignment?.startsAt.toMillis() ?? 0) - (right.assignment?.startsAt.toMillis() ?? 0)), [applicants]);
  ...
  return <div className="space-y-4"><div className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]"><aside className="rounded-3xl bg-white p-4 shadow-sm"><div><h3 className="font-black text-navy">배정 대기 지원자</h3><p className="mt-1 text-xs leading-5 text-slate-400">가능시간 응답 완료 · 현재 면접시간 없음</p>{!draft && <button type="button" onClick={onRunAutoAssignment} className="mt-3 w-full rounded-xl bg-navy px-3 py-2.5 text-[10px] font-black text-white shadow-sm transition-colors hover:bg-slate-800">미배정자 전체 자동배정</button>}</div><div className="mt-4 space-y-2">{visibleWaiting.map(applicant => { const autoAssigning = autoAssigningApplicantId === applicant.id; return <article key={applicant.id} className="rounded-2xl border border-slate-100 p-3"><div className="flex items-center justify-between gap-2"><div><p className="text-sm font-black text-navy">{applicant.name}</p><p className="text-[10px] text-slate-400">{applicant.applicantNumber} · {applicant.access?.availability.length ?? 0}개 응답</p></div><button type="button" disabled={autoAssigningApplicantId !== null} onClick={() => runApplicantAutoAssignment(applicant.id)} className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-2 text-[10px] font-black shadow-sm transition-colors duration-150 disabled:cursor-wait disabled:opacity-75 ${autoAssigning ? 'border-gold bg-gold/20 text-navy shadow-[0_0_0_3px_rgba(212,175,55,0.16)]' : 'border-gold/50 bg-navy text-white hover:border-gold hover:bg-slate-800'}`}><Bot size={14} className={autoAssigning ? 'text-navy' : 'text-gold'} />자동배정</button></div><button type="button" onClick={() => { setManualApplicantId(current => current === applicant.id ? null : applicant.id); setManualChoice(''); }} className="mt-2 text-[10px] font-bold text-slate-500">수동 배정 {manualApplicantId === applicant.id ? '닫기' : '열기'}</button>{manualApplicantId === applicant.id && <div className="mt-2 space-y-2"><select value={manualChoice} onChange={event => setManualChoice(event.target.value)} className="w-full rounded-xl border border-slate-200 px-2 py-2 text-[10px]"><option value="">공통 가능시간 선택</option>{manualOptions.map(option => <option key={option.value} value={option.value}>{option.slot.replace('|', ' ')} · {option.interviewerName}</option>)}</select><button type="button" disabled={!manualChoice} onClick={async () => { const [interviewerId, slot] = manualChoice.split('|||'); if (interviewerId && slot && await onAssign(applicant, slot, interviewerId)) { setManualApplicantId(null); setManualChoice(''); } }} className="w-full rounded-xl bg-navy px-3 py-2 text-[10px] font-black text-white disabled:opacity-40">수동 배정</button></div>}</article>; })}{visibleWaiting.length < waiting.length && <button type="button" onClick={() => setVisibleWaitingCount(current => current + SCHEDULE_LIST_BATCH)} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-[10px] font-black text-navy">대기자 더 보기 · {visibleWaiting.length}/{waiting.length}명</button>}{waiting.length === 0 && <p className="rounded-2xl border border-dashed border-slate-200 px-3 py-8 text-center text-xs text-slate-400">배정 대기자가 없습니다.</p>}</div></aside><section className="min-w-0 space-y-4 rounded-3xl bg-white p-4 shadow-sm">{draft && <AutoAssignmentPanel compact round={round} applicants={applicants} interviewers={interviewers} draft={draft} onRun={() => undefined} onDraftChange={onDraftChange} onApply={onApplyDraft} />}<WeeklySchedule round={round} applicants={applicants} assigned={assigned} interviewers={interviewers} draft={draft} actionNeededIds={openChangeRequestApplicantIds} onDraftChange={onDraftChange} onAssign={onAssign} onClearAssignment={onClearAssignment} onChangeAssignmentState={onChangeAssignmentState} onResetSchedule={onResetSchedule} /><MobileScheduleList assigned={assigned} actionNeededIds={openChangeRequestApplicantIds} onClearAssignment={onClearAssignment} onChangeAssignmentState={onChangeAssignmentState} onResetSchedule={onResetSchedule} /></section></div></div>;
}
