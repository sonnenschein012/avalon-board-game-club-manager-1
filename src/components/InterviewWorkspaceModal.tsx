...
export default function InterviewWorkspaceModal({
  applicant,
  round,
  interviewer,
  onClose,
  ...
  const completeInterview = async () => {
    ...
    try {
      ...
      const completed = await onComplete(applicant.id, {
        ...
        expectedNoteRevision: flushed.revision,
      });
  ...
  const requestCompletion = () => {
    if (!note.overallRating) {
      setValidationError('종합평가를 선택해야 면접을 완료할 수 있습니다.');
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-slate-950/60 backdrop-blur-sm md:p-5">
      <div className="mx-auto flex h-full w-full max-w-6xl flex-col overflow-hidden bg-slate-50 shadow-2xl md:rounded-3xl">
        <header className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-100 bg-white px-4 py-3 md:px-6 md:py-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gold">Interview workspace</p>
            <h2 className="text-lg font-black text-navy md:text-xl">
              {applicant.name} <span className="text-sm font-medium text-slate-400">{applicant.applicantNumber}</span>
            </h2>
          ...
          <div className="space-y-4">
            <section className="rounded-2xl bg-white p-4 shadow-sm">
              <h3 className="flex items-center gap-2 text-sm font-black text-navy">
                <BookOpen size={16} className="text-gold" />지원서 답변
              </h3>
              <dl className="mt-4 space-y-4">
                {applicant.applicationData.map((field, index) => (
                  <div key={`${field.header}-${index}`}>
                    <dt className="text-[10px] font-black text-slate-400">{field.header || `항목 ${index + 1}`}</dt>
                    <dd className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-700">{field.value || '-'}</dd>
                  </div>
                ))}
                {applicant.applicationData.length === 0 && (
                  <p className="text-sm text-slate-400">저장된 지원서 답변이 없습니다.</p>
                )}
              </dl>
            </section>
            ...
            <section className="rounded-2xl bg-white p-4 shadow-sm">
              <h3 className="flex items-center gap-2 text-sm font-black text-navy">
                <Clock3 size={16} className="text-gold" />지원자 가능시간
              </h3>
              <div className="mt-3 space-y-2">
                {availability.map(row => (
                  <div key={row.dateKey} className="grid grid-cols-[82px_1fr] gap-2 rounded-xl bg-indigo-50 px-3 py-2 text-xs">
                    <strong className="text-navy">{row.dateLabel}</strong>
                    <span className="text-slate-600">{row.ranges.join(', ')}</span>
                  </div>
                ))}
                {availability.length === 0 && <p className="text-sm text-slate-400">제출된 가능시간이 없습니다.</p>}
              </div>
            </section>
          </div>
          ...
          <div className="space-y-4">
            <section className="rounded-2xl bg-white p-4 shadow-sm">
              ...
              {note.state === 'conflict' && (
                <div role="alert" className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3">
                  <p className="text-xs font-bold leading-5 text-amber-900">
                    다른 운영진이 이 면접 기록을 수정했습니다. 현재 입력은 보존되어 있으며 자동 저장을 잠시 멈췄습니다.
                  </p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={note.acceptRemote}
                      className="rounded-lg bg-white px-3 py-2 text-[10px] font-black text-amber-800 shadow-sm"
                    >
                      서버의 최신 내용 불러오기
                    </button>
                    <button
                      type="button"
                      onClick={() => void note.overwriteRemote()}
                      className="rounded-lg bg-amber-700 px-3 py-2 text-[10px] font-black text-white"
                    >
                      내 입력으로 덮어쓰기
                    </button>
                  </div>
              ...
              {note.state === 'error' && (
                <button
                  type="button"
                  onClick={() => void note.retrySave()}
                  className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-[10px] font-black text-red-700"
                >
                  저장 다시 시도
                </button>
              )}
              <div className="mt-4 space-y-4">
                {questions.map((question, index) => (
                  <label key={question.id} className="block">
                    <span className="block whitespace-pre-wrap text-xs font-black leading-5 text-navy">
                      {index + 1}. {question.text}
                    </span>
                    <textarea
                      value={note.answers[question.id] ?? ''}
                      onChange={event => note.setAnswer(question.id, event.target.value)}
                      className="mt-2 min-h-28 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm leading-6 focus:border-gold focus:outline-none"
                      placeholder="면접 중 답변과 관찰 내용을 기록하세요."
                    />
                  </label>
                ))}
                {questions.length === 0 && (
                  <p className="rounded-xl bg-amber-50 px-3 py-3 text-xs font-bold text-amber-700">
                    회차 설정에 면접 질문을 추가하면 이곳에 질문별 입력란이 표시됩니다.
                  </p>
                )}
              </div>
            </section>
            ...
            <section className="rounded-2xl bg-white p-4 shadow-sm">
              <label className="block">
                <span className="text-sm font-black text-navy">종합 노트</span>
                <textarea
                  value={note.generalNotes}
                  onChange={event => note.setGeneralNotes(event.target.value)}
                  className="mt-3 min-h-48 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm leading-6 focus:border-gold focus:outline-none"
                  placeholder="질문 외 추가 메모, 후속 확인 사항 등을 자유롭게 기록하세요."
                />
              </label>
            </section>

            <section id="overall-rating" className="rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4">
              <h3 className="flex items-center gap-2 text-sm font-black text-navy">
                <Award size={16} className="text-gold" />면접관 종합평가 <span className="text-red-500">필수</span>
              </h3>
              <p className="mt-1 text-xs text-slate-500">평가 근거는 질문별 기록이나 종합 노트에 남겨주세요.</p>
              <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">
                {OVERALL_RATINGS.map(value => (
                  <button
                    ...
                    key={value}
                    onClick={() => {
                      note.setOverallRating(value);
                      setValidationError(null);
                    }}
                    className={`min-h-12 rounded-xl border px-2 py-2 text-xs font-black transition ${note.overallRating === value ? 'border-navy bg-navy text-white' : 'border-indigo-100 bg-white text-slate-600 hover:border-indigo-300'}`}
                  >
              ...
              {validationError && (
                <p role="alert" className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-xs font-bold text-red-600">
                  {validationError}
                </p>
              )}
            </section>
        ...
        <footer className="shrink-0 border-t border-slate-200 bg-white px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] md:px-6">
          {actionFormOpen && (
            <div className="mb-3 rounded-2xl border border-amber-200 bg-amber-50 p-3">
              <label className="block text-xs font-black text-amber-900">
                조치가 필요한 이유 <span className="font-medium text-amber-700">(선택)</span>
                <textarea
                  autoFocus
                  value={actionReason}
                  maxLength={500}
                  ...
                  placeholder="불참, 일정 재조율 등 필요한 조치를 적어주세요."
                />
              </label>
          ...
          {completionConfirmOpen && (
            <div className="mb-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-3">
              <p className="text-xs font-bold leading-5 text-emerald-900">
                {applicant.name} 지원자의 종합평가와 면접 기록을 저장하고 완료 처리할까요?
              </p>
              <div className="mt-2 flex justify-end gap-2">
                <button
                  type="button"
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-4 py-2.5 text-xs font-black text-amber-800 disabled:opacity-40"
            >
              <AlertTriangle size={15} />조치 필요
            </button>
            <button
              type="button"
              disabled={submitting || interviewCompleted}
            ...
            >
              ...
              {interviewCompleted ? '면접 완료됨' : '평가와 함께 면접 완료'}
            </button>
          </div>
        </footer>
      </div>
