...
import { Award, Check, CheckCircle2, Loader2, MessageSquareText, X, XCircle } from 'lucide-react';
...
export default function SelectionDetailModal({ applicant, round, onClose, onUpdateSelectionStatus, onUpdateOverallRating }: SelectionDetailModalProps) {
  ...
  return <div role="dialog" aria-modal="true" className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/60 p-3 backdrop-blur-sm md:p-5">
    <div className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-slate-50 shadow-2xl">
      ...
      <main className="grid flex-1 gap-4 overflow-y-auto p-4 md:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] md:p-6">
        ...
        <section className="rounded-2xl bg-white p-5 shadow-sm"><h3 className="flex items-center gap-2 text-sm font-black text-navy"><MessageSquareText size={16} className="text-gold" />질문별 면접 기록</h3><div className="mt-4 space-y-3">{(round.interviewQuestions ?? []).map((question, index) => <div key={question.id} className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-black text-navy">{index + 1}. {question.text}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{note.answers[question.id] || '기록 없음'}</p></div>)}{!(round.interviewQuestions ?? []).length && <p className="text-sm text-slate-400">등록된 면접 질문이 없습니다.</p>}</div></section>
      </main>
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-white px-5 py-4"><p className="text-xs font-bold text-slate-500">현재 선발 상태: <span className="text-navy">{SELECTIONS[selection]}</span></p><div className="flex gap-2"><DecisionButton label="미결정" active={selection === 'pending'} disabled={updating} onClick={() => void updateStatus('pending')} icon={<Check size={14} />} /><DecisionButton label="미선발" active={selection === 'rejected'} disabled={updating} onClick={() => void updateStatus('rejected')} icon={<XCircle size={14} />} /><DecisionButton label="선발" active={selection === 'selected'} disabled={updating} onClick={() => void updateStatus('selected')} icon={updating ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} /></div></footer>
    </div>
  </div>;
}
