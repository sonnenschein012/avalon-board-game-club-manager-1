...
import { CalendarDays, ChevronRight, Loader2, Plus, Trash2, Users } from 'lucide-react';
...
export default function InterviewRoundsPage() {
  const { rounds, countsByRound, loading, saving, saveRound, deletingRoundId, removeRound } = useInterviewRoundsLogic();
  ...
  return <div className="space-y-6">
    ...
    {loading ? <div className="flex justify-center py-24 text-slate-300"><Loader2 className="animate-spin" /></div> : rounds.length === 0 ? <div className="rounded-3xl border-2 border-dashed border-slate-200 bg-white py-24 text-center"><CalendarDays className="mx-auto text-slate-300" /><p className="mt-3 text-sm font-bold text-slate-500">아직 생성된 면접 회차가 없습니다.</p></div> : <div className="grid gap-4 lg:grid-cols-2">{rounds.map(round => {
      ...
      return <article key={round.id} className="group rounded-3xl border border-slate-100 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg md:p-6">
        <div className="flex items-start justify-between gap-3"><div className="min-w-0"><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black ${surveyStatus.collecting ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>{surveyStatus.label}</span><h2 className="mt-3 truncate text-lg font-black text-navy">{round.name}</h2><p className="mt-1 text-xs text-slate-400">{configured ? `기존 회차 기본 마감 ${round.surveyClosesAt.toDate().toLocaleString('ko-KR')}` : '회차 안에서 면접 일정을 추가해주세요.'}</p></div><button type="button" onClick={() => setDeleteTarget(round)} aria-label={`${round.name} 삭제`} className="rounded-xl p-2.5 text-slate-400 transition hover:bg-red-50 hover:text-red-600"><Trash2 size={16} /></button></div>
    ...
    <InterviewRoundDeleteModal open={Boolean(deleteTarget)} round={deleteTarget} applicantCount={deleteTarget ? countsByRound[deleteTarget.id]?.total ?? 0 : 0} deleting={deleteTarget?.id === deletingRoundId} onClose={() => { if (!deletingRoundId) setDeleteTarget(null); }} onConfirm={() => { if (!deleteTarget) return; void removeRound(deleteTarget).then(deleted => { if (deleted) setDeleteTarget(null); }); }} />
  </div>;
}
