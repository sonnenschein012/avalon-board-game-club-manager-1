...
export default function InterviewersPanel({ round, interviewers, applicants, onAdd, onSaveAvailability, onRemove }: Props) {
  ...
  return <div className="grid min-w-0 gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
    <aside className="min-w-0 space-y-4 rounded-3xl bg-white p-4 shadow-sm sm:p-5"><div><h3 className="font-black text-navy">면접관</h3><p className="mt-1 text-xs text-slate-400">이름으로 명시적으로 구분하며 이메일 연결은 선택 사항입니다.</p></div>
      ...
      <div className="grid gap-2 border-t border-slate-100 pt-4 sm:grid-cols-2 xl:grid-cols-1"><input value={name} onChange={event => setName(event.target.value)} placeholder="면접관 이름" className="min-w-0 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs" /><input value={email} onChange={event => setEmail(event.target.value)} placeholder="로그인 이메일 (선택)" className="min-w-0 w-full rounded-xl border border-slate-200 px-3 py-2 text-xs" /><button disabled={!name.trim()} onClick={async () => { if (await onAdd(name, email || undefined)) { setName(''); setEmail(''); } }} className="flex w-full items-center justify-center gap-1 rounded-xl bg-gold px-3 py-2.5 text-xs font-black text-navy disabled:opacity-40 sm:col-span-2 xl:col-span-1"><Plus size={14} />면접관 추가</button></div>
    </aside>
